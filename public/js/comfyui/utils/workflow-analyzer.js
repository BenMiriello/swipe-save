...
window.comfyUIServices.workflowAnalyzer = {
   ...
   * @param {Object} guiWorkflow - GUI format workflow
   * @returns {Object} API format workflow
   */
  convertGUIToAPI(guiWorkflow) {
    ...
    for (const node of guiWorkflow.nodes) {
      ...
      // Add widget values as inputs using hard-coded mappings
      if (node.widgets_values && Array.isArray(node.widgets_values)) {
        const inputNames = this.getInputNamesForNodeType(node.type);
      ...
      if (guiWorkflow.links && Array.isArray(guiWorkflow.links)) {
        for (const link of guiWorkflow.links) {
          if (link && Array.isArray(link) && link.length >= 5 && link[2] === node.id) {
            // link format: [id, origin_id, target_id, origin_slot, target_slot]
            const sourceNodeId = link[1];
            const sourceSlot = link[3];
            const targetSlot = link[4];
            
            // Get input name for target slot
            const inputNames = this.getInputNamesForNodeType(node.type);
            const inputName = inputNames[targetSlot];
  ...
  /**
   * Get input names for a specific node type
   * This requires node-type specific knowledge for common nodes
   * @param {string} nodeType - Node type
   * @returns {Array} Array of input names
   */
  getInputNamesForNodeType(nodeType) {
    const inputMappings = {
      ...
      'ImpactWildcardEncode': ['wildcard_text', 'populated_text', 'mode', 'seed', 'Select to add Wildcard', 'Select to add LoRA', 'Select to add Embedding'],
